...
// Import data và config từ file riêng
import { LOCATIONS, FOODS, LocationItem, FoodItem } from "./data";
import { sendTelegramMessage } from "./config";

// --- Types & Interfaces (WeatherType vẫn giữ local vì chỉ dùng trong App) ---
...
const getWeatherConfig = (code: number): WeatherType => {
  ...
  if (code === 0 || code === 1) {
    return {
      text: "Trời quang, đi chơi thôi Judy ơi! ✨",
      color: "text-blue-500",
      icon: <Star size={20} className="animate-spin-slow text-yellow-400" />,
    };
  }
  if (code === 2 || code === 3) {
    return {
      text: "Mát mẻ như ở Tundra Town! ☁️",
      color: "text-slate-500",
      icon: <Cloud size={20} className="animate-bounce text-slate-400" />,
    };
  }
  if (code >= 45 && code <= 48) {
    return {
      text: "Sương mù mờ ảo lắm nha! 🌫️",
      color: "text-purple-500",
      icon: <Cloud size={20} className="text-purple-300" />,
    };
  }
  if ((code >= 51 && code <= 67) || (code >= 80 && code <= 82)) {
    return {
      text: "Mưa rơi như ở Rainforest District! ☔",
      color: "text-blue-600",
      icon: <CloudRain size={20} className="animate-pulse text-blue-400" />,
    };
  }
  if (code >= 95) {
    return {
      text: "Sấm chớp! Trú mưa cùng Nick thôi! ⚡",
      color: "text-yellow-600",
      icon: (
        <CloudLightning size={20} className="animate-pulse text-yellow-500" />
      ),
    };
  }
  if (code >= 71) {
    return {
      text: "Lạnh buốt! Cần bộ lông ấm áp! ❄️",
      color: "text-cyan-600",
      icon: <Snowflake size={20} className="animate-spin text-cyan-400" />,
    };
  }

  // Default fallback
  return {
    text: "Thời tiết Zootopia hôm nay bí ẩn! ❤️",
    color: "text-pink-500",
    icon: <Sun size={20} className="text-pink-400" />,
  };
};
...
// Floating hearts animation for extra cuteness
...
const FloatingBackground = () => {
  return (
    <div className="fixed inset-0 overflow-hidden pointer-events-none -z-10 bg-gradient-to-br from-blue-50 via-purple-50 to-orange-50">
      ...
      {/* Floating Icons */}
      <div className="absolute top-[10%] left-[5%] text-orange-400 animate-float opacity-60">
        <Carrot size={48} fill="#fdba74" />
      </div>
      <div className="absolute top-[20%] right-[10%] text-slate-400 animate-float-delayed opacity-40">
        <PawPrint size={32} fill="#cbd5e1" />
      </div>
      <div className="absolute top-[50%] left-[8%] text-green-400 animate-float opacity-50">
        <Leaf size={40} fill="#86efac" />
      </div>
      <div className="absolute bottom-[20%] right-[5%] text-pink-300 animate-float opacity-50">
        <Heart size={28} fill="#f9a8d4" />
      </div>
      <div className="absolute bottom-[10%] left-[15%] text-blue-300 animate-float-delayed opacity-60">
        <Cloud size={56} fill="#bae6fd" />
      </div>
      <div className="absolute top-[15%] left-[40%] text-yellow-400 animate-spin-slow opacity-50">
        <Star size={24} fill="#fde047" />
      </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 h-40 opacity-20 text-slate-600 flex items-end">
        ...
        >
          <path
            fill="currentColor"
            d="M0,120 L0,60 Q150,80 300,60 T600,60 T900,60 T1200,60 L1200,120 Z"
            opacity="0.4"
          />
          <path
            fill="currentColor"
            d="M0,120 L100,120 L100,50 L150,50 L150,120 L250,120 L250,80 L350,80 L350,120 L500,120 L500,40 L600,40 L600,120 L800,120 L800,60 L900,60 L900,120 L1100,120 L1100,70 L1200,70 L1200,120 Z"
          />
        </svg>
      </div>
    </div>
...
const Mascot = ({ step }: { step: number }) => {
  ...
  if (step === 1) {
    // Cute bunny - Planning date
    gifUrl = "https://media.giphy.com/media/aQGqcObSxfixy/giphy.gif";
    message = "Hí hí~ Lên kèo hẹn hò đi nào! 💕🐰";
    borderColor = "border-pink-400";
  } else if (step === 2) {
    // Cute food cat - Yummy time
    gifUrl = "https://media.giphy.com/media/mlvseq9yvZhba/giphy.gif";
    message = "Nyam nyam~ Ăn gì ngon không? 🍩😋";
    borderColor = "border-yellow-400";
  } else if (step === 3) {
    // Cute waiting animation
    gifUrl = "https://media.giphy.com/media/tHIRLHtNwxpjIFqPdV/giphy.gif";
    message = "Đợi tí nha~ Quà đang tới... 🎁✨";
    borderColor = "border-purple-400";
  } else {
    // Love celebration
    gifUrl = "https://media.giphy.com/media/26BRv0ThflsHCqDrG/giphy.gif";
    message = "Yay! Hẹn hò chốt đơn rồi nè! 💖🎉";
  ...
  return (
    ...
    >
      <div className={`relative ${poked ? "animate-wiggle" : ""}`}>
        <div
          className={`w-36 h-36 rounded-full border-[6px] shadow-xl overflow-hidden bg-white ${borderColor}`}
        >
          <img
            src={gifUrl}
            alt="Zootopia Mascot"
        ...
        {poked && (
          <>
            <div className="absolute -top-2 -right-6 text-3xl animate-bounce z-20">
              🐰
            </div>
            <div className="absolute top-0 -left-6 text-3xl animate-pulse z-20 delay-100">
              🦊
            </div>
          </>
        )}
      ...
      <div
        className={`mt-3 px-5 py-2.5 rounded-2xl shadow-sm border border-white/60 transition-colors duration-300 backdrop-blur-md ${
          poked ? "bg-orange-100 text-orange-600" : "bg-white/80 text-slate-700"
        }`}
      >
        <p className="font-heading font-bold text-lg text-center leading-tight">
          {poked ? "Sly fox! Dumb bunny! 🥰" : message}
        </p>
      </div>
    </div>
  );
...
const Button = ({
  ...
  const baseStyles =
    "px-6 py-3 rounded-2xl font-heading font-bold transition-all duration-200 transform active:scale-90 flex items-center gap-2 justify-center shadow-lg hover:-translate-y-1 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:translate-y-0 disabled:active:scale-100";

  const variants = {
    // Modified primary to be more Zootopia-ish (Judy Blue to Nick Orange)
    primary:
      "bg-gradient-to-r from-blue-400 to-orange-400 text-white hover:from-blue-500 hover:to-orange-500 shadow-orange-200/50 border-b-4 border-orange-500 active:border-b-0 active:mt-1",
    secondary:
      "bg-white text-slate-600 hover:bg-slate-50 border-2 border-slate-100 shadow-slate-100/50",
    outline: "border-2 border-blue-300 text-blue-500 hover:bg-blue-50",
    ghost:
      "bg-transparent text-slate-500 shadow-none hover:bg-slate-100/50 border-none",
  };
    ...
    <div className="flex justify-center items-center gap-3 mb-4">
      {[1, 2].map((step) => (
        <div key={step} className="flex items-center">
          ...
          {step < 2 && (
            <div
              className={`w-8 h-1.5 rounded-full mx-2 transition-all duration-500 ${
                step < currentStep ? "bg-blue-300" : "bg-slate-100"
              }`}
            />
          )}
        </div>
...
const App = () => {
    ...
    const fetchWeather = async () => {
      ...
      try {
        ...
        if (data.hourly && data.hourly.time && data.hourly.time.length > 0) {
          ...
          let index = data.hourly.time.findIndex((t: string) =>
            t.startsWith(targetHourStr)
          );
  ...
  const handleNext = async () => {
    ...
    if (step === 2) {
      ...
      // 🔔 Gửi thông báo Telegram khi submit form
      const locations = allLocations.filter((l) =>
        selectedLocations.includes(l.id)
      );
      const foods = FOODS.filter((f) => selectedFoods.includes(f.id));
...
📍 <b>Địa điểm (${locations.length}):</b>
${locations.map((l) => `  • ${l.emoji} ${l.name}`).join("\n")}

🍽️ <b>Món ăn (${foods.length}):</b>
${foods.map((f) => `  • ${f.name}`).join("\n")}${
        selectedFoods.includes("custom") && customFoodName
          ? `\n  • ✨ ${customFoodName}`
          : ""
      }

${loveNote ? `💌 <b>Lời nhắn:</b>\n"${loveNote}"` : ""}
  ...
  const renderStep1 = () => (
    <div className={`space-y-8 animate-slide-up`}>
      <div className="text-center space-y-1">
        <h2 className="text-3xl font-heading font-bold text-slate-700">
          Lên kèo đi chơi! ✨
        </h2>
        <p className="text-slate-500 font-medium">
          Chọn ngày đẹp & địa điểm cùng Judy nào
        </p>
      </div>
      ...
      <div className="bg-white/90 backdrop-blur-md p-6 rounded-[2rem] shadow-xl shadow-blue-100/50 border-2 border-white transform transition-transform">
        <label className="block text-slate-600 font-heading font-bold mb-3 flex items-center gap-2 text-lg">
          <Calendar className="text-blue-400 animate-bounce" size={24} /> Chọn
          ngày
        </label>
        ...
        <div className="mt-4 min-h-[60px]">
          {isLoadingWeather ? (
            <div className="flex items-center justify-center gap-2 p-3 text-slate-400 bg-slate-50 rounded-xl border border-dashed border-slate-200">
              <Loader2 size={20} className="animate-spin" /> Flash đang tra cứu
              thời tiết... chậm... lắm...
            </div>
          ) : weather ? (
            <div className="p-3 bg-white rounded-xl border border-slate-100 flex items-center gap-3 animate-pop shadow-sm">
              ...
              <div className="flex flex-col">
                <p className={`font-heading font-bold ${weather.color}`}>
                  {weather.text}
                </p>
                {weather.temp !== undefined && (
                  <span className="text-xs text-slate-400 font-medium">
                    Dự báo khoảng {weather.temp}°C vào lúc 19:00
                  </span>
                )}
              </div>
            </div>
      ...
      <div className="bg-white/90 backdrop-blur-md p-6 rounded-[2rem] shadow-xl shadow-orange-100/50 border-2 border-white">
        <div className="flex justify-between items-center mb-6">
          <label className="text-slate-600 font-heading font-bold flex items-center gap-2 text-lg">
            <MapPin className="text-orange-400 animate-bounce" size={24} /> Chọn
            địa điểm
          </label>
  ...
  const renderStep2 = () => (
    <div className="space-y-8 animate-slide-up">
      <div className="text-center space-y-1">
        ...
        <p className="text-slate-500 font-medium">
          Clawhauser bảo món nào cũng ngon hết!
        </p>
      </div>
        ...
        {FOODS.map((item, index) => {
          ...
          return (
            ...
            >
              ...
              <div
                className={`p-5 ${isSelected ? "bg-yellow-50" : "bg-white"}`}
              >
          ...
          <div className="flex flex-col items-center text-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-slate-400 font-medium">
                Bánh Pawpsicle hay gì?
              </p>
            </div>

            {selectedFoods.includes("custom") && (
              <div
                className="w-full mt-2 animate-pop"
      ...
      <div className="flex justify-between pt-4 gap-4">
        ...
        <Button
          ...
          disabled={
            ...
            (selectedFoods.includes("custom") && !customFoodName.trim())
          }
  ...
  const renderStep3 = () => (
    <div className="flex flex-col items-center justify-center min-h-[400px] animate-slide-up text-center">
      ...
      <p className="text-slate-500 font-medium mb-8">
        {unwrapProgress < 5
          ? `Gõ cửa nhà Flash ${5 - unwrapProgress} lần nữa để lấy vé nhé!`
          : "Mở thôi!"}
      </p>
      ...
      <button
        ...
        className={`group relative transition-all focus:outline-none select-none transform
         ...
         `}
        style={{
          transform: `scale(${1 + unwrapProgress * 0.1}) rotate(${
            unwrapProgress % 2 === 0 ? 3 : -3
          }deg)`,
        }}
      >
        ...
        <Gift
          size={160}
          className={`text-orange-500 drop-shadow-2xl cursor-pointer z-10 relative transition-transform ${
            unwrapProgress > 0 ? "animate-wiggle" : ""
          }`}
          strokeWidth={1}
          fill="#fed7aa"
        />
  ...
  const renderStep4 = () => {
    ...
    return (
      <div className="space-y-6 animate-pop relative">
        ...
        <div
          ref={ticketRef}
          className="bg-white rounded-[2rem] shadow-2xl overflow-hidden relative mx-2 sm:mx-0 transform hover:scale-[1.01] transition-transform duration-500 border-4 border-blue-200"
        >
          ...
          <div className="bg-gradient-to-r from-blue-400 via-indigo-400 to-orange-400 p-6 text-center relative overflow-hidden">
            <div className="absolute top-0 left-0 w-full h-full opacity-20">
              <div className="absolute top-2 left-10 animate-pulse">
                <Carrot fill="white" size={20} />
              </div>
              <div className="absolute bottom-4 right-10 animate-pulse">
                <PawPrint fill="white" size={24} />
              </div>
            </div>
            <h3 className="text-white font-heading font-bold text-3xl tracking-widest uppercase drop-shadow-md">
              SUMMARY
            </h3>
            <div className="text-white/80 text-sm font-medium tracking-widest">
              ZOOTOPIA DATE TICKET
            </div>
          ...
          <div className="p-8 space-y-6 bg-white relative">
              ...
              <div>
                ...
                {weather && (
                  <div className="flex items-center gap-1.5 mt-1">
                    ...
                    <p className={`text-sm font-bold ${weather.color}`}>
                      {weather.text}
                    </p>
                  </div>
                )}
              ...
              <div className="flex-1">
                ...
                <div className="flex flex-wrap gap-2">
                  {locations.map((loc) => (
                    <span
                      key={loc.id}
                      className="bg-orange-50 px-3 py-1.5 rounded-xl text-slate-600 text-sm font-bold border border-orange-100 flex items-center gap-1.5"
                    >
              ...
              <div className="flex-1">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {standardFoods.map((food) => (
                    <div
                      key={food.id}
                      className="flex items-center gap-2 bg-yellow-50 p-2 rounded-lg border border-yellow-100"
                    >
                  ...
                  {selectedFoods.includes("custom") && (
                    <div className="flex items-center gap-2 bg-yellow-50 p-2 rounded-lg border border-yellow-100">
                      <div className="w-8 h-8 rounded-full bg-white flex items-center justify-center text-xs shrink-0 border border-yellow-200">
                        ✨
                      </div>
                      <span className="text-sm font-bold text-slate-700 truncate">
                        {customFoodName}
            ...
            {loveNote && (
              <div className="mt-4 p-4 bg-orange-50 rounded-xl relative transform rotate-1 border border-orange-100 shadow-sm">
                <div className="absolute -top-3 -left-2 text-red-400 transform -rotate-12">
              ...
              <div className="space-y-1">
                <div className="h-8 w-48 bg-slate-200 rounded-sm opacity-50 flex items-center justify-center gap-1 overflow-hidden">
                  ...
                  {[...Array(20)].map((_, i) => (
                    <div
                      key={i}
                      className="h-full bg-slate-400 w-0.5"
                      style={{
                ...
                <div className="text-[10px] text-slate-400 font-mono tracking-[0.2em]">
                  ZPD-TICKET-{new Date().getFullYear()}
                </div>
              </div>
      ...
      <div className="max-w-xl mx-auto relative z-10">
        ...
        <div
          className={`transition-all duration-500 transform ${
            isAnimate ? "scale-95 opacity-50" : "scale-100 opacity-100"
          }`}
        >
          {step === 1 && renderStep1()}
          {step === 2 && renderStep2()}
        ...
        <footer className="mt-12 text-center text-slate-400 text-sm font-medium pb-8">
          <p className="flex items-center justify-center gap-2">
            Made with{" "}
            <Heart size={12} fill="currentColor" className="text-orange-400" />{" "}
            for my dumb bunny
          </p>
        </footer>
      </div>
